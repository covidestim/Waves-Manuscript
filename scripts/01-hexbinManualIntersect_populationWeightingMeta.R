...
args <- list()
args$county_polygons <- "Data/data-sources/county_polygons.topojson"
args$hexsize <- 25
args$lower_48 <- T
...
metapop <- st_read("Data/data-sources/hexgrid_meta30m_population.geojson") %>% 
...
if(args$modelVersion == "preomicron"){
  observations <- read_csv(
    "Data/data-sources/covidestim-observations.csv",
    col_types = cols(
      fips         = col_character(),
      date         = col_date(),
      cases        = col_number(),
  ...
  observations <- vroom::vroom(
    "Data/data-sources/omicronera_observation.csv")
  
  #### Also set a test date for the plots throughout the workflow
  testDate <- "2022-01-20"
  
} else if(args$modelVersion == "immunity"){
  observations <- vroom::vroom("Data/data-sources/fips-immunity.csv")
  
  #### Also set a test date for the plots throughout the workflow
  testDate <- "2020-07-26"
} else{ 
  observations <- vroom::vroom("Data/data-products/observations_fullmodel.csv") |> 
...
tempPop$hexid <- as.numeric(tempPop$hexid) ### used for filtering

### Plot the population across the hexes 
ggplot() + geom_sf(tempPop, mapping=aes(fill=population)) +
  scale_fill_gradient(low = "thistle1", high = "deeppink4") +
  geom_sf(tempPop %>% filter(population == 0), mapping=aes(), fill="green")
...
hexObservationsAllNoMissingGeom <- geojson_read("Data/data-products/hexid-observations_preomicron05-31-25.geojson")

## Read the csv and join with the metapop hexgrid

tmpCSVnoGeom <- vroom::vroom("Data/data-products/hexid-observations_preomicron05-31-25NOGEOM.csv")

metapop <- sf::st_read("Data/data-sources/hexgrid_meta30m_population.geojson")

tmpBoth <- dplyr::full_join(tmpCSVnoGeom, metapop %>% 
                       dplyr::select(!metapop_30m), 
                     by=c("hexid")) %>% 
  sf::st_as_sf()

sf::st_write(obj = tmpBoth,
         dsn = "Data/data-products/hexid-observations_preomicron_meta30m.geojson",
         delete_dsn = T,
         delete_layer = T)
...
hexTest <- hexObservationsAllNoMissingGeom %>% 
...
st_write(obj = hexObservationsAllNoMissingGeom, 
         paste0("data-products/geo-hexes/hexid-observations_", args$modelVersion, "05-30-25.geojson"),
         delete_dsn = T)
